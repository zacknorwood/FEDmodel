...
equation
           ...
           eq_dcpowerflow5  slack angle constraint
           eq_dcpowerflow6  slack voltage constraint
...
eq_dcpowerflow1(h,Bus_IDs)..((e_imp_AH(h) - e_exp_AH(h))/1000)$(ord(Bus_IDs)=5)-((el_VKA1(h) + el_VKA4(h))/1000)$(ord(Bus_IDs)=10)
            +eta_Inverter/1000* (sum(BID, (exist_PV_cap_roof(BID) * PV_power_roof(h,BID))$BusToBID(Bus_IDs,BID))+
            sum(BID, (exist_PV_cap_facade(BID) * PV_power_facade(h,BID))$BusToBID(Bus_IDs,BID)))
            +eta_Inverter/1000* (sum(r, (PV_roof_cap_Inv(r) * PV_power_roof(h,r))$BusToBID(Bus_IDs,r))+
            sum(f, (PV_facade_cap_Inv(f) * PV_power_facade(h,f))$BusToBID(Bus_IDs,f)))+
            ((BES_dis(h)*BES_dis_eff - BES_ch(h)/BES_ch_eff)/1000)$(ord(Bus_IDs)=13)+(e_TURB(h)/1000)$(ord(Bus_IDs)=7)+
            ((BFCh_dis(h)*BFCh_dis_eff - BFCh_ch(h)/BFCh_ch_eff)/1000)$(ord(Bus_IDs)=16)-sum(i_AH_el,el_demand(h,i_AH_el)$BusToB_ID(Bus_IDs,i_AH_el))/1000
...
eq_dcpowerflow2(h,Bus_IDs)..(re_imp_AH(h)/1000)$(ord(Bus_IDs)=5)-0.2*sum(i_AH_el,el_demand(h,i_AH_el)$BusToB_ID(Bus_IDs,i_AH_el))/1000
...
eq_dcpowerflow3(h,Bus_IDs,j)$(currentlimits(Bus_IDs,j) ne 0)..(delta(h,Bus_IDs)-delta(h,j))*Y(Bus_IDs,j)=l=currentlimits(Bus_IDs,j);

eq_dcpowerflow4(h,Bus_IDs,j)$(currentlimits(Bus_IDs,j) ne 0)..(delta(h,Bus_IDs)-delta(h,j))*Y(Bus_IDs,j)=g=-currentlimits(Bus_IDs,j);

eq_dcpowerflow5(h)..delta(h,"13")=e=0;

eq_dcpowerflow6(h)..V(h,"13")=e=1;
*-----------------Comments on El Network implementation------------------------*
$ontext
1)Havent not include Studentbostader
2)el_RM(h),e_RMMC(h),e_AAC(h), e_HP(h),e_RMInv(h) are not included
3)inverter of FBch is the same with the pv additional constraints must be added
4)current limit constraints need correction
$offtext
