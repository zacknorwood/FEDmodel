function [e_demand_measured, h_demand_measured,c_demand_measured,...
          ...
          irradiance_measured] = fread_measurments(t_init, t_len)
%This function is used to read mesurment between the indicated indices

demand_range=strcat('B',int2str(2+t_init),':AJ',int2str(2+t_init+t_len));
gen_range=strcat('B',int2str(4+t_init),':B',int2str(4+t_init+t_len-1)); 
price_ramge=strcat('B',int2str(2+t_init),':B',int2str(2+t_init+t_len-1));
tout_ramge=strcat('B',int2str(2+t_init),':B',int2str(2+t_init+t_len-1));%'B2:B100';
irradiance_range=strcat('B',int2str(2+t_init),':AJ',int2str(2+t_init+t_len-1));
